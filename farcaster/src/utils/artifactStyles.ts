...
// Get combined card styles based on rarity
export function getCardStyles(rarity: string = 'common') {
  const style = RARITY_STYLES[rarity] || RARITY_STYLES.common
  return `${style.bg} ${style.border} ${style.glow || ''}`
}

// Get name text style based on rarity
export function getNameStyles(rarity: string = 'common') {
  const style = RARITY_STYLES[rarity] || RARITY_STYLES.common
  return style.text
}

// Helpers for backward compat
export const AGE_COLORS = Object.fromEntries(
  Object.entries(AGE_STYLES).map(([k, v]) => [k, v.style])
)
export const MATERIAL_COLORS = Object.fromEntries(
  Object.entries(MATERIAL_STYLES).map(([k, v]) => [k, v.style])
)
