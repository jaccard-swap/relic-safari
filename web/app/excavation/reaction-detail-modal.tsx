...
export function ReactionDetailModal({ reaction, onClose, chainId }: ReactionDetailModalProps) {
  ...
  return (
    <InfoModal open={true} onClose={onClose} title="Fusion Result" icon="⚗️">
      <div className="mb-4">
        <div className="mb-1.5 text-[13px] text-stone-500">Target Artifact</div>
      ...
      {upgrades.length > 0 && (
        <div className="mb-4">
          <div className="mb-1.5 text-[13px] text-stone-500">Trait Upgrades</div>
      ...
      {experience.length > 0 && (
        <div className="mb-4">
          <div className="mb-1.5 text-[13px] text-stone-500">Experience Gained</div>
          <div className="space-y-1.5">
            {experience.map(([key, xp]) => (
              <div key={key} className="flex items-center justify-between rounded border border-amber-700/30 bg-amber-950/30 p-2">
                <span className="text-[13px] capitalize text-stone-400">{key}</span>
                <span className="text-[13px] font-medium text-amber-300">+{xp} XP</span>
              </div>
            ))}
          </div>
          <div className="mt-1.5 text-[11px] text-stone-500">Experience accumulates toward the next trait level</div>
        </div>
      )}

      {reaction.essenceYield > 0 && (
        <div className="mb-4">
          <div className="mb-1.5 text-[13px] text-stone-500">Essence Extracted</div>
          <div className="rounded border border-purple-700/30 bg-purple-950/30 p-3 text-center">
            <span className="text-lg text-purple-300">+{reaction.essenceYield} ✨</span>
            <div className="mt-1 text-[11px] text-stone-500">From non-matching traits on consumed artifact</div>
          </div>
        </div>
      )}
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="rounded bg-stone-800/50 p-2 text-center">
          ...
          <div className="text-[11px] text-stone-500">Upgrades</div>
        </div>
        <div className="rounded bg-stone-800/50 p-2 text-center">
          <div className="text-sm text-amber-400">{experience.length}</div>
          <div className="text-[11px] text-stone-500">XP Gains</div>
        </div>
        <div className="rounded bg-stone-800/50 p-2 text-center">
          <div className="text-sm text-purple-400">{reaction.essenceYield}</div>
          <div className="text-[11px] text-stone-500">Essence</div>
        </div>
      </div>
