...
export function ReactionCard({ reaction, onClick }: ReactionCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded border border-purple-900/30 bg-purple-950/20 p-2 ${
        onClick ? "cursor-pointer transition-colors hover:border-purple-700/50 hover:bg-purple-950/40" : ""
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="truncate text-xs font-medium text-purple-200">⚗️ {name}</div>
          <div className="mt-1 flex flex-wrap gap-1.5">
            {upgrades.map(([key, { from, to }]) => (
              <span key={key} className="rounded border border-emerald-700/30 bg-emerald-950/50 px-1.5 py-1 text-[11px] text-emerald-300">
                ⬆ {key}: {from} → {to}
              </span>
            ))}
            {experience.map(([key, xp]) => (
              <span key={key} className="rounded border border-amber-700/30 bg-amber-950/50 px-1.5 py-1 text-[11px] text-amber-300">
                +{xp} {key} XP
              </span>
            ))}
            {reaction.essenceYield > 0 && (
              <span className="rounded border border-purple-700/30 bg-purple-950/50 px-1.5 py-1 text-[11px] text-purple-300">
                +{reaction.essenceYield} ✨
              </span>
            )}
          </div>
        </div>
        <div className="whitespace-nowrap text-[11px] text-stone-500">{timeAgo}</div>
      </div>
    </div>
  );
}
