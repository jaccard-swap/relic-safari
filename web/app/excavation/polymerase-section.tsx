...
import { FuseModal } from "./fuse-modal";
...
export function PolymeraseSection({ expanded, onToggle, onHelp, onReactionsHelp }: PolymeraseSectionProps) {
  ...
  return (
    <>
      ...
      >
        {targetNft && (
          <div className="mt-3 rounded-lg border border-purple-500/20 bg-gradient-to-br from-stone-900 via-purple-950/20 to-stone-900 p-3">
            <div className="mb-3 flex items-center justify-between">
              <span className="bg-gradient-to-r from-emerald-400 via-cyan-400 to-purple-400 bg-clip-text text-xs font-semibold text-transparent">
                ◈ FUSION WORKBENCH
              </span>
              <button type="button" onClick={clearSelection} className="text-[11px] text-stone-500 hover:text-red-400">
                Clear
              </button>
            </div>
            ...
            {consumedNft ? (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="rounded border border-purple-700/30 bg-purple-900/30 p-2 text-purple-300 transition-colors hover:bg-purple-800/50 hover:text-purple-200"
            ...
            {!consumedNft && <div className="mt-3 text-center text-xs text-purple-300/70">Select catalyst artifact below ↓</div>}
          </div>
        )}
        ...
        <div className="mt-4 border-t border-purple-500/20 pt-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-xs font-medium text-purple-300">🧪 Recent Reactions</span>
            <button type="button" onClick={onReactionsHelp} className="text-[13px] text-stone-500 hover:text-purple-300">
              ?
            </button>
          </div>
          {reactionsLoading ? (
            <div className="animate-pulse py-3 text-center text-xs text-stone-500">Loading...</div>
          ) : reactions.length === 0 ? (
            <div className="py-3 text-center text-xs text-stone-500">No fusions yet</div>
          ) : (
