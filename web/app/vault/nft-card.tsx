  ...
  return (
    <div className="rounded overflow-hidden">
      <button
        type="button"
        onClick={onToggle}
        className={`w-full flex items-center gap-1.5 p-1.5 border transition-all text-left ${cardStyles} ${isExpanded ? "rounded-t border-b-0" : "rounded"}`}
      >
        ...
        <button type="button" onClick={handleDetails} className="text-[9px] text-white/30 hover:text-amber-300 shrink-0 px-1">
          ?
        </button>
        <div className="text-[7px] text-white/30 shrink-0">{getChainName(nft.chainId)}</div>
        ...
        <div className="overflow-hidden">
          <div className={`p-1.5 border border-t-0 rounded-b ${cardStyles}`}>
            ...
            <div className="flex gap-1">
              <button type="button" onClick={handleAuction} className="flex-1 py-1 bg-amber-700/80 hover:bg-amber-600 text-white text-[9px] font-medium rounded transition-colors">
                🏛️ Auction
              </button>
              <button type="button" disabled className="flex-1 py-1 bg-stone-600/30 text-stone-500 text-[9px] font-medium rounded cursor-not-allowed">
                ⚗️ Fuse
              </button>
              <button type="button" disabled className="flex-1 py-1 bg-stone-700/30 text-stone-500 text-[9px] font-medium rounded cursor-not-allowed">
                ↗ Gift
              </button>
            </div>
          </div>
